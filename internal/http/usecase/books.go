...
func NewBook() *Book {
	return &Book{
		Books:  make(map[int64]gen.Pet),
		NextId: 1000,
	}
}

func (p *Book) FindBooks(ctx echo.Context, params gen.FindBooksParams) error {
	p.Lock.Lock()
	defer p.Lock.Unlock()

	var result []gen.Pet

	for _, pet := range p.Books {
		if params.Tags != nil {
...
func (p *Book) AddBook(ctx echo.Context) error {
...
func (p *Book) FindBookById(ctx echo.Context, id int64) error {
	p.Lock.Lock()
	defer p.Lock.Unlock()

	pet, found := p.Books[id]
	if !found {
...
func (p *Book) DeleteBook(ctx echo.Context, id int64) error {
	p.Lock.Lock()
	defer p.Lock.Unlock()
